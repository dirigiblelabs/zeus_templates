...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
		if (id) {
			var entity = deploymentContainersDao.get(id);
			if (entity !== null) {
			...
			var deploymentContainersCount = deploymentContainersDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', deploymentContainersCount);
		} else if (metadata !== null) {
			var deploymentContainersMetadata = deploymentContainersDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(deploymentContainersMetadata, null, 2));
		} else {
			var deploymentContainers = deploymentContainersDao.list(limit, offset, sort, desc, deploymentTemplateId);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(deploymentContainers, null, 2));
		}
	}
}
...
function handlePostRequest(httpRequest, httpResponse) {
	...
	var id = deploymentContainersDao.create(entity);
...
function handlePutRequest(httpRequest, httpResponse) {
	...
	if (id !== null) {
		if (deploymentContainersDao.get(id) !== null) {
			deploymentContainersDao.update(entity);
...
function handleDeleteRequest(httpRequest, httpResponse, xss) {
	...
	if (id !== null) {
		var entity = deploymentContainersDao.get(id);
		if (entity !== null) {
			deploymentContainersDao.delete(entity);
			sendResponse(httpResponse, httpResponse.NO_CONTENT);
		} else {
			sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'id\'=' + id);
		}
