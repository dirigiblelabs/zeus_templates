...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
		if (id) {
			var entity = applicationDao.get(id);
			if (entity !== null) {
			...
			var applicationCount = applicationDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', applicationCount);
		} else if (metadata !== null) {
			var applicationMetadata = applicationDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(applicationMetadata, null, 2));
		} else {
			var application = applicationDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(application, null, 2));
		}
	}
}
...
function handlePostRequest(httpRequest, httpResponse) {
	...
	var id = applicationDao.create(entity);
