...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(null, count, metadata, httpResponse)) {
		if (count !== null) {
			var serviceTypesCount = serviceTypesDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', serviceTypesCount);
		} else if (metadata !== null) {
			var serviceTypesMetadata = serviceTypesDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(serviceTypesMetadata, null, 2));
		} else {
			var serviceTypes = serviceTypesDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(serviceTypes, null, 2));
		}
	}
}
