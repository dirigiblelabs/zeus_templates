...
var typesDao = require('zeus/templates/dao/typesDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(null, count, metadata, httpResponse)) {
		if (count !== null) {
			var typesCount = typesDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', typesCount);
		} else if (metadata !== null) {
			var typesMetadata = typesDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(typesMetadata, null, 2));
		} else {
			var types = typesDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(types, null, 2));
		}
	}
}
