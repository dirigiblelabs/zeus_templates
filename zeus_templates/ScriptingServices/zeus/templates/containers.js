...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
		if (id) {
			var entity = containersDao.get(id);
			if (entity !== null) {
			...
			var containersCount = containersDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', containersCount);
		} else if (metadata !== null) {
			var containersMetadata = containersDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(containersMetadata, null, 2));
		} else {
			var containers = containersDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(containers, null, 2));
		}
	}
}
...
function handlePostRequest(httpRequest, httpResponse) {
	...
	var id = containersDao.create(entity);
...
function handlePutRequest(httpRequest, httpResponse) {
	...
	if (id !== null) {
		if (containersDao.get(id) !== null) {
			containersDao.update(entity);
